...
function App() {
  ...
  const addTrack = (track) => {
    ...
    const trackExists =  playlistTracks.filter(playlistItem => playlistItem.id === track.id);
  ...
  const removeTrack = (track) => {
    ...
    const uniqueTracks  = playlistTracks.filter(playlistItem => playlistItem.id !== track.id);
